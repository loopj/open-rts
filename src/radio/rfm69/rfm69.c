...
void rfm69_set_data_mode(rfm69_t *radio, int mode) {
    spi_write_masked(radio->spi_module, RFM69_REG_DATA_MODUL, RFM69_DATA_MODE, mode);
}

void rfm69_set_modulation_type(rfm69_t *radio, int modulation) {
    spi_write_masked(radio->spi_module, RFM69_REG_DATA_MODUL, RFM69_MODULATION_TYPE, modulation);
}
...
void rfm69_set_rx_bandwidth(rfm69_t *radio, int rxBw) {
    ...
    spi_write_masked(radio->spi_module, RFM69_REG_RX_BW, RFM69_RX_BW_MANT | RFM69_RX_BW_EXP, bwMant | bwExp);
}
...
void rfm69_set_transmit_power(rfm69_t *radio, int power) {
    if(radio->use_pa_boost) {
        if(power <= 10) {
            ...
            spi_write(radio->spi_module, RFM69_REG_PA_LEVEL, RFM69_PA1_ON | (power + 18));
        } else if(power <= 17) {
            // 13 to 17 dBm, both PAs required
            spi_write(radio->spi_module, RFM69_REG_PA_LEVEL, RFM69_PA1_ON | RFM69_PA2_ON | (power + 14));
        } else {
            // 18 - 20 dBm, both PAs and high power settings required
            spi_write(radio->spi_module, RFM69_REG_PA_LEVEL, RFM69_PA1_ON | RFM69_PA2_ON | (power + 11));
        }
    } else {
        // low power module, use only PA0
        spi_write(radio->spi_module, RFM69_REG_PA_LEVEL, RFM69_PA0_ON | (power + 18));
    }
...
void rfm69_set_mode(rfm69_t *radio, int mode) {
    ...
    // while(!(spi_read(radio->spi_module, RFM69_REG_IRQ_FLAGS_1) & RFM69_MODE_READY));
