...
{
    spi_write_masked(
        radio->spi_module, RFM95_REG_OP_MODE, RFM95_LONG_RANGE_MODE,
        long_range_mode ? RFM95_LONG_RANGE_MODE_ON : RFM95_LONG_RANGE_MODE_OFF);
}

void sx1278_set_data_mode(struct sx1278 *radio, int data_mode)
{
    spi_write_masked(radio->spi_module, RFM95_REG_PACKET_CONFIG_2,
                     RFM95_DATA_MODE, data_mode);
}

void sx1278_set_modulation_type(struct sx1278 *radio, int modulation_type)
{
    spi_write_masked(radio->spi_module, RFM95_REG_OP_MODE,
                     RFM95_MODULATION_TYPE, modulation_type);
}

void sx1278_set_bit_sync_on(struct sx1278 *radio, bool bit_sync_on)
{
    spi_write_masked(radio->spi_module, RFM95_REG_OOK_PEAK, RFM95_BIT_SYNC,
                     bit_sync_on ? RFM95_BIT_SYNC_ON : RFM95_BIT_SYNC_OFF);
}
...
void sx1278_set_rx_bandwidth(struct sx1278 *radio, int rxBw)
{
    ...
    spi_write_masked(radio->spi_module, RFM95_REG_RX_BW,
                     RFM95_RX_BW_MANT | RFM95_RX_BW_EXP, bwMant | bwExp);
}
...
void sx1278_set_transmit_power(struct sx1278 *radio, int power)
{
    if (radio->use_pa_boost) {
        ...
        spi_write_masked(radio->spi_module, RFM95_REG_PA_CONFIG,
                         RFM95_PA_SELECT, RFM95_PA_SELECT_PA_BOOST);

        if (power == 20) {
            // Enable +20dBm option
            spi_write_masked(radio->spi_module, RFM95_REG_PA_DAC, RFM95_PA_DAC,
                             RFM95_PA_DAC_ENABLED);

            // Set output power
            spi_write_masked(radio->spi_module, RFM95_REG_PA_CONFIG,
                             RFM95_OUTPUT_POWER, 15);
        } else {
            // Disable the +20dBm option
            spi_write_masked(radio->spi_module, RFM95_REG_PA_DAC, RFM95_PA_DAC,
                             RFM95_PA_DAC_DISABLED);

            // Set output power
            spi_write_masked(radio->spi_module, RFM95_REG_PA_CONFIG,
                             RFM95_OUTPUT_POWER, power - 2);
        }
    } else {
        // Transmit using RFO pins
        spi_write_masked(radio->spi_module, RFM95_REG_PA_CONFIG,
                         RFM95_PA_SELECT, RFM95_PA_SELECT_RFO);
        ...
        if (power < 0) {
            // Set output power
            spi_write_masked(radio->spi_module, RFM95_REG_PA_CONFIG,
                             RFM95_MAX_POWER, RFM95_MAX_POWER_LOW);
            spi_write_masked(radio->spi_module, RFM95_REG_PA_CONFIG,
                             RFM95_OUTPUT_POWER, power + 3);
        } else {
            // Set output power
            spi_write_masked(radio->spi_module, RFM95_REG_PA_CONFIG,
                             RFM95_MAX_POWER, RFM95_MAX_POWER_MAX);
            spi_write_masked(radio->spi_module, RFM95_REG_PA_CONFIG,
                             RFM95_OUTPUT_POWER, power);
        }
    }
}
...
void sx1278_set_mode(struct sx1278 *radio, int mode)
{
    ...
    while (!(spi_read(radio->spi_module, RFM95_REG_IRQ_FLAGS_1) &
             RFM95_MODE_READY))
        ;
