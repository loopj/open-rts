...
void rts_frame_init(rts_frame_t *frame, uint8_t command, uint16_t rolling_code, uint32_t address) {
...
void rts_frame_from_bytes(rts_frame_t *frame, uint8_t *bytes) {
    ...
    frame->remote_address = bytes[4] | (uint32_t)bytes[5] << 8 | (uint32_t)bytes[6] << 16;
}
